...
function MarathonEventBusClient (options) {
    ...
    if (options.marathonHeaders && Object.getOwnPropertyNames(options.marathonHeaders).length > 0) {
        Object.getOwnPropertyNames(options.marathonHeaders).forEach(function (key) {
    ...
    // Add connectionId event if enabled
    if (self.options.enableConnectionEvent) {
        self.options.eventTypes.push("connectionId");
    }

    // Usable to check if the function argument is a function itself
...
MarathonEventBusClient.prototype.subscribe = function () {
    ...
    // Allow custom headers if set
    if (self.options.marathonHeaders && Object.getOwnPropertyNames(self.options.marathonHeaders).length > 0) {
        eventSourceInitDict.headers = self.options.marathonHeaders;
    }
